...
export const FinanceView: React.FC = () => {
  ...
  const fetchFinanceData = async () => {
    ...
    try {
      ...
      const safeFetch = async (promise: Promise<any>, fallbackValue: any) => {
        try {
      ...
      const [invRes, expRes, catRes] = await Promise.all([
        safeFetch(supabase.from('invoices').select('*, companies(name)').order('date', { ascending: false }), []),
          ...
          <div key={inv.id} className="flex items-center justify-between p-3 hover:bg-slate-50 dark:hover:bg-white/[0.02] rounded-2xl transition-all group border border-transparent hover:border-slate-200 dark:hover:border-white/5">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">#{inv.id.substring(0,8)} • {inv.date}</p>
              </div>
            </div>
                 ...
                 <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-8 gap-1.5">
                        {presetColors.map(color => (
                          <button
                            ...
                            className={`h-6 rounded-md transition-all border-2 flex items-center justify-center ${
                              categoryFormData.color === color ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'
                            }`}
                    ...
                    <div className="flex gap-2 pt-2">
                       ...
                       <button 
                          ...
                          className={`flex-[2] py-2.5 rounded-lg text-[9px] font-black uppercase text-white shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2 tracking-[0.15em] ${
                            saveSuccess ? 'bg-emerald-600 shadow-emerald-900/40' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-900/40'
                          }`}
                       >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5">
                       {recurrenceOptions.map((opt) => (
                         <button
                           ...
                           className={`py-1.5 rounded-md text-[8px] font-black uppercase tracking-widest transition-all border ${
