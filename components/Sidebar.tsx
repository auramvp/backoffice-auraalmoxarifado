...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside 
      className={`bg-white dark:bg-[#0A0D14] flex flex-col border-r border-slate-200 dark:border-white/5 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] relative ${
    ...
    >
      <div className={`pt-6 px-4 mb-6 flex items-center justify-between overflow-hidden whitespace-nowrap`}>
        ...
        <button 
          ...
          className={`p-2 rounded-lg border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-gray-500 hover:text-blue-600 dark:hover:text-white hover:bg-white dark:hover:bg-white/10 transition-all backdrop-blur-md ${
            isCollapsed ? 'mx-auto' : ''
          }`}
        >
      ...
      <nav className="flex-1 px-3 space-y-1 mt-2">
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center rounded-xl transition-all duration-300 group overflow-hidden whitespace-nowrap ${
              activeView === item.id 
